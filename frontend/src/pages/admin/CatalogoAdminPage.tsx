...
export function CatalogoAdminPage() {
  ...
  const iniciarEdicao = (s: CatalogoServicoAdmin) => {
    setEditId(s.id);
    setForm({ nome: s.nome, precoMinimo: Number(s.precoMinimo), precoTexto: s.precoTexto || '', descricao: s.descricao || '', pontos: s.pontos, garantiaDias: s.garantiaDias, ativo: s.ativo });
  };
  ...
  const desativar = async (s: CatalogoServicoAdmin) => {
    if (!confirm(`Desativar "${s.nome}" do catálogo?`)) return;
    try {
              ...
              <tr key={s.id} className="border-t">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    ...
                    {isAdmin && s.ativo && (
                      <Button variant="danger" onClick={() => desativar(s)}>Desativar</Button>
                    )}
                  </div>
                </td>
              </tr>
