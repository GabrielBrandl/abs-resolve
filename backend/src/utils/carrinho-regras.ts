...
/** Valor mínimo quando o carrinho contém serviços (visita técnica). */
export const MINIMO_CARRINHO_SERVICO = 150;

/** Peças avulsas: abaixo deste valor cobra-se taxa de entrega. */
...
export function analisarItensCarrinho(
  ...
  for (const item of itens) {
    ...
    const unit = resolverPreco ? resolverPreco(item) : Number(item.precoUnitario || 0);
    const subtotal = unit * item.quantidade;
    if (isPecaItem(item.slug, item.tipo)) {
      subtotalPecas += subtotal;
      temPeca = true;
    } else {
...
export function calcularTaxaEntrega(endereco: EnderecoEntrega): { taxa: number; regiao: string } {
  ...
  if (uf && uf !== 'am') {
    throw new Error('Entrega de peças avulsas disponível apenas no Amazonas.');
  }
  ...
  if (cidade && cidade !== 'manaus' && INTERIOR_AM.some((c) => cidade.includes(c) || c.includes(cidade))) {
    return { taxa: 45, regiao: `Interior AM — ${endereco.cidade}` };
  }

  if (cidade && cidade !== 'manaus' && uf === 'am') {
  ...
  if (cidade.includes('manaus') || !cidade) {
    return { taxa: 20, regiao: 'Manaus' };
  }

  return { taxa: 38, regiao: endereco.cidade || 'Amazonas' };
}
