...
export type CrmModo = 'b2c' | 'b2b';

const EMPTY_LEAD_B2B = {
  nome: '',
  segmento: 'Farmácia',
  telefone: '',
  bairro: '',
...
export function CRMBoard({ modo }: { modo: CrmModo }) {
  const isB2B = modo === 'b2b';
  const tipoLeadFixo = isB2B ? 'prospeccao_b2b' : 'inbound';
  const emptyLead = isB2B ? EMPTY_LEAD_B2B : EMPTY_LEAD_B2C;
  ...
  const [novoLead, setNovoLead] = useState({ ...emptyLead });
  ...
  const [leadForm, setLeadForm] = useState({
    ...
    telefone: '',
    email: '',
    origem: isB2B ? 'prospeccao_b2b' : 'whatsapp',
    tipoLead: tipoLeadFixo,
    segmento: '',
    bairro: '',
    ligou: '',
    atendeu: '',
  ...
  const paramsFiltro = useMemo(() => {
    const p: Record<string, string> = { tipoLead: tipoLeadFixo };
    ...
    return p;
  }, [filtros, competencia, mesesDb, tipoLeadFixo]);
  ...
  const abrirLeadPorId = async (id: string) => {
    try {
      ...
      const leadEhB2B = full.tipoLead === 'prospeccao_b2b';
      if (leadEhB2B !== isB2B) {
  ...
  const criarLead = async () => {
    ...
    try {
      await leadsApi.criar({
        ...
        origem: isB2B ? 'prospeccao_b2b' : novoLead.origem,
        tipoLead: tipoLeadFixo,
  ...
  return (
    <div>
      <PageHeader
        title={isB2B ? 'CRM B2B' : 'CRM B2C'}
        subtitle={
          isB2B
            ? 'Prospecção ativa — campos da planilha, fila e triagem'
            : 'Pipeline inbound — site, WhatsApp, campanhas e catálogo'
        }
        action={<Button onClick={abrirNovoLead}>{isB2B ? 'Novo Lead B2B' : 'Novo Lead'}</Button>}
      />
        ...
        {isB2B ? (
          ...
          <>
            <Select label="Origem" value={filtros.origem} onChange={(e) => setFiltros({ ...filtros, origem: e.target.value })}>
              ...
              {ORIGENS_LEAD.filter((o) => o.key !== 'prospeccao_b2b').map((o) => (
                          ...
                          {(prov) => (
                            ...
                            >
                              ...
                              {isB2B ? (
                                ...
                                <p className="mt-1 text-xs text-slate-600">
                                  {lead.catalogoServico?.nome || lead.interesse || lead.origem || 'Sem serviço'}
      ...
      <Modal
        ...
        title={isB2B ? 'Novo Lead B2B' : 'Novo Lead B2C'}
        wide
      >
        {isB2B ? (
          <>
